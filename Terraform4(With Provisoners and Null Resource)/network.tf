...
resource "aws_instance" "instance" {
  ...
  vpc_security_group_ids = [ aws_security_group.sec_grp["sg-1"].id ] 
  ...
  provisioner "local-exec" {
    when = destroy
    command = "scp -i id_rsa ubuntu@${self.public_ip}:/home/ubuntu/config-file ./"
...
resource "null_resource" "name" {
  ...
  provisioner "remote-exec" {
    ...
    user = "ubuntu"
    private_key = file("~/.ssh/id_rsa")
    host = aws_instance.instance["ec2-1"].public_ip
  }
    inline = [
        file("script.sh")
    ]
